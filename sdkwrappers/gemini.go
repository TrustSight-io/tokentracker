...
func (w *GeminiSDKWrapper) ExtractTokenUsageFromResponse(response interface{}) (TokenUsage, error) {
	// Try to cast the response to *genai.GenerateContentResponse
	resp, ok := response.(*genai.GenerateContentResponse)
	if !ok {
		return TokenUsage{}, fmt.Errorf("response is not a *genai.GenerateContentResponse: %T", response)
	}

	// Check if usage metadata is available
	if resp.UsageMetadata == nil {
		return TokenUsage{}, fmt.Errorf("response does not contain usage metadata")
	}
...
func (w *GeminiSDKWrapper) FetchCurrentPricing() (map[string]tokentracker.ModelPricing, error) {
